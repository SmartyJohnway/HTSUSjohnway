...
var refresh_hts_exports = {};
__export(refresh_hts_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(refresh_hts_exports);
...
var handler = async () => {
  ...
  const hts = chunks.map((x) => ({
    ...
    desc: x.description,
    uoq: x.uoq,
    general: x.general_rate,
    special: x.special_rate,
    col2: x.column2_rate
  }));
  await store.setJSON("hts/latest.json", { updatedAt: (/* @__PURE__ */ new Date()).toISOString(), items: hts });
...
//# sourceMappingURL=refresh-hts.js.map
