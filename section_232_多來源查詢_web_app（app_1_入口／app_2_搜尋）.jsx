 ...
 * - App4：索引搜尋 Beta（**移除外部依賴，改用內建 MinimalSearchEngine**）
 *
 * 修正：
 * - 先前 App4 動態載入 `flexsearch`，環境禁止從 CDN 取模組導致建置失敗。
 * - 現改為內建 MinimalSearchEngine（零外部依賴）。
 *
 * 備註：
 * - 前端不保存任何 API 金鑰；需要金鑰的來源請用 Netlify Functions 代理。
 * - 若要啟用 App4 的實際資料檢索，請在 Netlify Functions 產生 search/index.json 並存入 Netlify Blobs。
 */

// ====== 工具：輕量全文索引（零依賴） ======
/** Tokenize：
 * - 英數：保留字母/數字與點（支援 9903.81.90），並切出以點分隔的片段
 * - CJK：建立 bigram（連續兩字）以提升檢索
 */
function tokenize(input: string): string[] {
  ...
  const asciiParts = keepDots
    ...
    .flatMap(t => {
      ...
      const parts = [t];
      if (t.includes('.')) {
        parts.push(...t.split('.').filter(Boolean));
      }
      return parts;
    })
    .filter(Boolean);
...
/** MinimalSearchEngine：
 * - fields: { name, weight }
 * - add(doc)：建立倒排索引 {token -> Map(docId -> scoreContribution)}
 * - search(q)：根據 token 匹配分數加總 + 簡易短語加權，輸出按分數排序的 doc 陣列
 */
  ...
  return (
    <div className="sticky top-0 z-20 backdrop-blur bg-white/70 border-b">
      <div className="max-w-6xl mx-auto px-4 py-3 flex items-center gap-3">
        ...
        <div className="text-xs text-gray-500 ml-auto">v2.1 · React + Tailwind（已移除外部模組依賴）</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="rounded-2xl shadow p-5 border bg-white flex flex-col gap-3">
      ...
      <div className="flex flex-wrap gap-2">
        <a href={src.home} target="_blank" rel="noreferrer" className="px-3 py-1.5 rounded-xl border hover:bg-gray-50">開啟首頁</a>
...
function App4() {
  const [endpoint, setEndpoint] = useState("/.netlify/blobs/data/search/index.json");
  ...
  const runTests = () => {
    ...
    t.push({ name: '英文關鍵字 AND/OR（ERW steel pipe）', pass: r2.some(d=>d.id==='HTS-9903.81.90'), detail: r2.map(d=>d.id).join(',') });
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6">
      <div className="mb-2">
        ...
        <p className="text-sm text-gray-600">從後端排程（Netlify Functions）產生的 <code>search/index.json</code> 載入到前端，使用 MinimalSearchEngine 快速檢索。預設端點指向 Netlify Blobs。</p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {results.map((r:any)=> (
            <div key={r.id} className="rounded-xl border p-4 bg-white shadow">
              ...
              <div className="text-sm text-gray-700 line-clamp-3">{r.text}</div>
              {r.type === 'fr' && r.extra?.html_url && (
                <a className="mt-2 inline-block text-sm px-3 py-1.5 rounded-xl border hover:bg-gray-50" href={r.extra.html_url} target="_blank" rel="noreferrer">Federal Register</a>
              )}
            </div>
          ))}
        </div>
      ...
      <div className="mt-8 text-xs text-gray-500 space-y-2">
        <div>⚙️ 後端建議：使用 Netlify Scheduled Functions 產生 <code>search/index.json</code>（見我們提供的 sample）。</div>
        <div>🔐 機密：Regulations.gov/USITC DataWeb 的金鑰與 token 僅放在 Functions 的環境變數。</div>
      </div>
    </div>
  );
}
